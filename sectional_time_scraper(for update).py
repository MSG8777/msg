...
DEFAULT_START_DATE = "2008/04/02"
MAX_WORKERS = 8

# --- NEW: INCREMENTAL HELPER ---
def get_start_date(file_path):
    """
    Checks the CSV for the latest date. 
    Returns the next day as a datetime object.
    """
    if not os.path.exists(file_path):
        print(f"No existing data found. Starting from default: {DEFAULT_START_DATE}")
        return datetime.strptime(DEFAULT_START_DATE, "%Y/%m/%d")
    
    try:
        # Read only the 'Date' column to save memory
        df_existing = pd.read_csv(file_path, usecols=['Date'], parse_dates=['Date'])
        if df_existing.empty:
            return datetime.strptime(DEFAULT_START_DATE, "%Y/%m/%d")
        
        last_date = df_existing['Date'].max()
        # Resume from the day after the last record
        start_from = last_date + timedelta(days=1)
        print(f"Resuming from last date in file: {last_date.strftime('%Y/%m/%d')}")
        return start_from
    except Exception as e:
        print(f"Error reading existing file: {e}. Starting from default.")
        return datetime.strptime(DEFAULT_START_DATE, "%Y/%m/%d")
    ...
    for race_no in range(1, 15):
        ...
        try:
            ...
            for _, row in target_df.iterrows():
                ...
                if isinstance(ft_val, pd.Series): ft_val = ft_val.iloc[-1]
                ft_val = str(ft_val).strip() if not pd.isna(ft_val) else None

                row_data = {
                    "Date": date_str, "RaceNo": race_no, "HorseNo": row.get('HorseNo'),
                    "HorseName": name_part, "BrandNo": brand_part, "FinishTime": ft_val
                }
                for i in range(1, 7):
...
def main():
    ...
    start_dt = get_start_date(DATA_FILE)
    ...
    print(f"--- HKJC SECTIONAL SCRAPER v13 (INCREMENTAL) ---")
    print(f"Scraping from: {start_dt.strftime('%Y/%m/%d')} to {today_dt.strftime('%Y/%m/%d')}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for i, future in enumerate(as_completed(future_to_date)):
            ...
            if i % 10 == 0:
                progress = (i / len(date_list)) * 100 if len(date_list) > 0 else 100
    ...
    print(f"\nDone. Total new rows added: {total_saved + len(batch_data)}")

...
# DEFAULT HISTORICAL START (Used only if no DB exists)
...
def worker(date_obj):
    ...
    # Summer Break logic (approximate)
    ...
    data = []
    try:
        # Check races 1 to 14 (Max possible races)
        for r in range(1, 15):
            ...
            if r_data:
                ...
                break
            # If we hit empty races after race 1, we stop checking deeper (e.g., usually only 8-12 races)
            elif r > 10 and not r_data:
                break
    except:
        pass
    finally:
...
def main():
    ...
    today = datetime.now()
    start_date_obj = datetime.strptime(DEFAULT_START_DATE, "%Y/%m/%d")

    # Check if DB exists to find the last date
    if os.path.exists(FINAL_DB_FILE) and os.path.getsize(FINAL_DB_FILE) > 0:
        try:
            # Read only the 'Date' column to find max date efficiently
            df_dates = pd.read_csv(FINAL_DB_FILE, usecols=['Date'])
            df_dates['Date'] = pd.to_datetime(df_dates['Date'], format="%Y/%m/%d", errors='coerce')
            last_date = df_dates['Date'].max()
            
            if pd.notna(last_date):
                print(f"Found existing database. Last date: {last_date.strftime('%Y-%m-%d')}")
                # Start from the NEXT day
                start_date_obj = last_date + timedelta(days=1)
            else:
                print("Database file exists but no valid dates found. Starting from default.")
        except Exception as e:
            print(f"Error reading existing DB ({e}). Starting from default.")
    else:
        print("No existing database found. Starting from scratch.")
    ...
    # Generate dates from Start -> Today (Forward)
    delta = (today - start_date_obj).days
    date_list = [start_date_obj + timedelta(days=x) for x in range(delta + 1)]
    
    print(f"--- HKJC VET SCRAPER (UPDATE MODE) ---")
    print(f"Scanning from {start_date_obj.strftime('%Y/%m/%d')} to {today.strftime('%Y/%m/%d')}")
    print(f"Days to process: {len(date_list)}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for i, future in enumerate(as_completed(future_map)):
            ...
            # Progress bar
            if i % 10 == 0 or i == len(date_list) - 1:
                sys.stdout.write(f"\rProgress: {i+1}/{len(date_list)} days checked | New Records: {total_new + len(new_records)}")
                sys.stdout.flush()
                
            # Batch save to avoid memory issues (Appending)
            if len(new_records) >= 50:
                pd.DataFrame(new_records).to_csv(FINAL_DB_FILE, mode='a', header=False, index=False)
    ...
    print(f"\nScraping finished. Added {total_new} new records.")

    # --- 3. FINAL SORT (OLD to NEW) ---
    print("Sorting database (Oldest -> Newest)...")
    try:
        df_full = pd.read_csv(FINAL_DB_FILE)
        # Ensure 'Date' is datetime for correct sorting
        df_full['Date_dt'] = pd.to_datetime(df_full['Date'], format="%Y/%m/%d", errors='coerce')
        
        # Sort
        df_full = df_full.sort_values(by=['Date_dt', 'RaceNo'], ascending=[True, True])
        
        # Drop temp column and duplicates
        df_full = df_full.drop(columns=['Date_dt'])
        df_full = df_full.drop_duplicates(subset=['Date', 'RaceNo', 'HorseName'], keep='last')
        
        # Save overwriting the file
        df_full.to_csv(FINAL_DB_FILE, index=False)
        print("Database successfully sorted and saved.")
        
    except Exception as e:
        print(f"Error during sorting phase: {e}")
